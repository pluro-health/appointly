...
import AppointlyCancelButton from "@calcom/web/components/booking/AppointlyCancelButton";
import CancelBooking from "@calcom/web/components/booking/CancelBooking";
import EventReservationSchema from "@calcom/web/components/schemas/EventReservationSchema";
import { timeZone } from "@calcom/web/lib/clock";
import { useAppointlyCancellationInfo } from "@calcom/web/lib/hooks/useAppointlyCancellationInfo";
...
export default function Success(props: PageProps) {
  ...
  const currentUserEmail =
    ...
    session?.user?.email ??
    bookingInfo?.attendees?.[0]?.email ?? // Fallback to primary attendee email
    undefined;
  ...
  // Appointly cancellation info hook
  const { cancellationInfo, isLoading: isCancellationLoading } = useAppointlyCancellationInfo(
    bookingInfo?.id || 0,
                    ...
                    <div
                      ...
                      <div
                        className={classNames(
                          ...
                          payment === "success" ? "bg-success" : "",
                          payment === "failed" ? "bg-error" : "",
                          !giphyImage && isReschedulable && !needsConfirmation && !payment
                            ? "bg-success"
                            : "",
                          !giphyImage && isReschedulable && needsConfirmation && !payment ? "bg-subtle" : "",
                          isCancelled && !payment ? "bg-error" : ""
                        )}>
                        {payment === "success" && (
                        ...
                        {!payment && !giphyImage && !needsConfirmation && isReschedulable && (
                          <Icon name="check" className="h-5 w-5 text-green-600 dark:text-green-400" />
                        )}
                        {!payment && needsConfirmation && isReschedulable && (
                    ...
                    <div className="mb-8 mt-6 text-center last:mb-0">
                      ...
                      {payment === "failed" && (
                        <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
                          <div className="flex items-center">
                            <div className="flex-shrink-0">
                              <Icon name="x" className="h-5 w-5 text-red-600 dark:text-red-400" />
                            </div>
                            <div className="ml-3">
                              ...
                              <div className="mt-2 text-sm text-red-700 dark:text-red-300">
                                <p>
                                  {reason
                                    ...
                                    : "Your payment was unsuccessful. Please book your appointment again."}
                                </p>
                              </div>
                              <div className="mt-4">
                                <Button
